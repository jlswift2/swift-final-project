            ...
            <form>
                {tags.map((value, index) => (
                    <div key={index}>
                        ...
                        {tags.length > 1 &&
                        (
                            <button type="button" className="font-serif w-1/8 text-center px-2 font-bold text-3xl text-red-200 hover:text-red-500 transition duration-300 ease-in-out" onClick={() => handleRemove(index)}>
                                <span>-</span>
                            </button>
                        )}
                        {tags.length - 1 === index &&
